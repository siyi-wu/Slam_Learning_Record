...
随着深度学习的发展，SurfaceNet和LSM率先尝试用神经网络解决多视图立体匹配。它们将整个 3D 空间划分成一个个体素（Voxel），把多视角的图像特征反投影到这个巨大的3D网格中，然后用3D卷积去分类每个体素是不是物体的表面。3D 网格的体积是随着分辨率呈立方级增长，这导致显存消耗巨大。同时由于分辨率受限于Voxel的大小，高分辨率与高显存消耗这一组trade-off导致这两种方法不能扩展到大规模重建。
...
传统方法（例如SGM）使用的是原始像素，而MVSNet先用CNN将图像转换为高维特征图。这里用到的是共享权重的特征提取方法，将原图的长宽都缩小为原图的1/4。这一步降采样实际上是为了后续降显存而做的。
...
在这一步之后，我们可以得到N个[H/4,W/4,D,32]的代价体。其中D是指D个深度平面。这个代价体里面记载的是：这么多像素，在32个特征下，D个深度的每个像素的特征。

### 代价度量

这里选择使用方差作为代价度量。方差可以直接测量特征差异，因此可以用于评估匹配相似度。比如某个地方被认为两个不同的深度都是正确的，此处方差就会增大，这可以为后续3D CNN提供信息。此处将N个代价体合为一个。
